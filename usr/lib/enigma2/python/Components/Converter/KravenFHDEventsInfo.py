...
class KravenFHDEventsInfo(Converter, object):
	...
	def getText(self):
		...
		if self.type < self.PrimeTime:
			...
			if curEvent:
				now = localtime(time())
				try:
	                           dt = datetime(now.tm_year, now.tm_mon, now.tm_mday, int(config.plugins.KravenFHD.Primetime.value[0]), int(config.plugins.KravenFHD.Primetime.value[1]))
                                except:
